...
- a **Timeline** section: each utterance as `**[MM:SS] <speaker>:** “<text>”` (curly quotes), with the events joined to it — the first utterance (in time) whose span, widened by the report's join window, contains the event — as indented bullets `[MM:SS] <kind> <selector> "<text>" value="…" (<route>)` (straight quotes); events matched by no utterance appear as standalone bullets in time order; every `MM:SS` on this page (including the header's duration) carries a leading `-` for a negative time — one preceding `t0` (a recording predating it, see `t`'s note above);
