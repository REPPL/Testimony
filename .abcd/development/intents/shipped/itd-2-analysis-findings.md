...
- Keyframe *extraction* (`ffmpeg -ss` at an utterance timestamp): needs local video and a multimodal pass, tied to Mode B/mapping, and deferred to a later intent (spc-2, flagged divergence). This slice covers the on-demand *request* only.
...
- **Given** an ambiguous utterance, **when** the analysis pass runs, **then** the rubric lets the model flag the verbal-only referent as needing a keyframe, at the request level (frame *extraction* is deferred; see What's Out of Scope).
