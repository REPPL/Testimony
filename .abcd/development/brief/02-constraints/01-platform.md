...
- **macOS is the primary target.** Capture depends on the Mac's recording
  affordances (ffmpeg's avfoundation backend, for both microphone and screen),
  and the code accounts for it explicitly — e.g. `transcribe`'s device
  auto-detection never selects CUDA on Darwin. A macOS app layer is planned to
  wrap the CLI core.
