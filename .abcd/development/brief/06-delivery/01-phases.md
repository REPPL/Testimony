...
| 2 — analysis layer | Rubric, finding schema, chunking, report generation; verification-pass workflow | Shipped ([`../01-product/04-analysis.md`](../01-product/04-analysis.md)): `analyze` and `review`. Chunking is a flagged divergence — v1 emits the whole timeline as one chunk, deferring real per-task chunking behind a seam |
