...
- Decodes each finding with unknown fields disallowed — a closed output shape, so
  ...
  and uniqueness, `t` within the session, the `type`, `severity`, and `mode`
  enums, non-empty `evidence` of at most 64 ids with every id real and at least
  one spoken `utt-*` anchor, a `quote` that is a verbatim substring of one
  *cited* evidence utterance's text, and any `ui` selector/route matching a real
  event.
