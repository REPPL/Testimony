...
// start/end reached mapSegments unbounded at the segment level: round2's
...
// refused where it enters rather than downstream. mapSegments rounds a word's
// own time through the same round2, so a word carries the identical risk;
// whisperx.go's word loop checks it at its own boundary instead of here,
// since an implausible word is dropped rather than refusing the segment.
