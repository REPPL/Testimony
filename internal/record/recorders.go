...
func probeDevices(ffmpeg string, wantScreen bool) (screenIndex int, mics []string, err error) {
	...
	// sandbox/exec denial, a fork failure after LookPath already succeeded) is a
	// different error: out is then empty, parseAVDevices finds nothing, and
	// selectDevices would misreport "no microphone found", sending the operator to
	// check hardware and permissions for a process that never started. Surface it.
	...
	if err != nil {
		...
		// in out, which parseAVDevices then reads as an empty listing. Without the
